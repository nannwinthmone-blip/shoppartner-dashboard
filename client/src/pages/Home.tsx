...
type CustomNotification = { id: number; title: string; detail: string; icon: string; tone: string; time: string; read: boolean };
...
const initialNotifications: CustomNotification[] = [
  { id: 1, title: "Commission posted", detail: "$12.40 from order SP-1048", icon: "💰", tone: "blue", time: "12 min ago", read: false },
  { id: 2, title: "Low stock alert", detail: "Lemon drop candle has 8 left", icon: "📦", tone: "peach", time: "1 hr ago", read: false },
  { id: 3, title: "Promotion is live", detail: "Fall Finds is reaching shoppers", icon: "✨", tone: "mint", time: "Yesterday", read: false },
];
...
export default function Home() {
  ...
  const [modal, setModal] = useState<"product" | "order" | "payout" | "promotion" | "support" | "notification" | null>(null);
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-area">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <div className="notification-wrap"><button className="notification-button" aria-label={`View notifications, ${unreadNotifications} unread`} onClick={() => setNotificationsOpen((open) => !open)}><Bell size={19} />{unreadNotifications > 0 && <span className="notification-dot" />}</button>{notificationsOpen && <div className="notification-popover"><div className="popover-header"><div><strong>Notifications</strong><span>{unreadNotifications ? `${unreadNotifications} unread update${unreadNotifications === 1 ? "" : "s"}` : "All caught up"}</span></div><button onClick={() => { setNotifications((current) => current.map((item) => ({ ...item, read: true }))); showToast("All caught up", "Notifications marked as read."); }}>Mark read</button></div>{notifications.slice(0, 4).map((item) => <button className={`popover-notification ${item.read ? "is-read" : ""}`} key={item.id} onClick={() => setNotifications((current) => current.map((entry) => entry.id === item.id ? { ...entry, read: true } : entry))}><span className={`notification-icon ${item.tone}`}>{item.icon}</span><div><strong>{item.title}</strong><span>{item.detail}</span><small>{item.time}</small></div></button>)}<button className="notification-compose-link" onClick={() => { setNotificationsOpen(false); setModal("notification"); }}><Send size={13} /> Create custom notification</button></div>}</div>
      ...
      {modal && <Modal type={modal} editingProduct={editingProduct} onClose={() => { setModal(null); setEditingProduct(null); }} onSaveProduct={handleSaveProduct} showToast={showToast} onCreateNotification={(notification) => { setNotifications((current) => [{ ...notification, id: Date.now(), time: "Just now", read: false }, ...current]); }} />}
